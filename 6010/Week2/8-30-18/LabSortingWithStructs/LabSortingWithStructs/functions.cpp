...
std::vector<card> sortHand(const std::vector<card>& inputDeck) {
    std::vector<card> fiveHand;
    for (int i = 0; i <5; i++) {
        fiveHand.push_back(inputDeck[i]);
    }

    for (int i = 0; i < fiveHand.size(); i++) {
        int minValueIndex = i;
        int minValue = fiveHand[i].rank;
        for (int j = i; j < fiveHand.size(); j++) {
            if (fiveHand[j].rank < minValue) {
                minValue = fiveHand[j].rank;
                minValueIndex = j;
            }

            card tempCard = fiveHand[i];
            fiveHand[i] = fiveHand[minValueIndex];
            fiveHand[minValueIndex] = tempCard;
        }
    }
    return fiveHand;
}
...
bool isStraight(const std::vector<card>& inputdeck) {
    std::vector<card> sortedDeck = sortHand(inputdeck);
    for (int i = 0; i < sortedDeck.size() - 1; i++) {
        if (sortedDeck[i].rank + 1 != sortedDeck[i+1].rank) {
            return false;
        }
    }
    return true;
...
bool isFullHouse(const std::vector<card>& inputDeck) {
    ...
    if (firstCardRank <2 || firstCardRank > 3) {
        return false;
    }
