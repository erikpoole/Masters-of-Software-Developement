 ...
 */
unsigned char GetByte(unsigned int input, int b)
{
    input <<= (3-b) * 8;
    input >>= 24;
 ...
 */
unsigned int SetByte(unsigned int input, unsigned char value, int b)
{
    int mask = 0xFF;
    mask <<= b * 8;
    mask = ~mask;
 ...
 * Do not use the * or *= operators
 * Do not use the / or /= operators
 * Do not use the + or += operators     (+1) is OK
 * Do not use the - or -= operators
...
int Negate(int input)
{
    ...
    //stolen from Challenge Question
    int maskLocation = 1;
    while (maskLocation & input) {
        int maskChanger = ~maskLocation;
        input = (input & maskChanger);
        maskLocation <<= 1;
    }
    input = (input | maskLocation);
...
int Increment(int x){
    ...
    int maskLocation = 1;                       // 0001
    while (maskLocation & x) {                  // While x[bit] = 1 at maskLocation
        int maskChanger = ~maskLocation;        // 1110
        x = (x & maskChanger);                  // change 1 to 0 at only masklocation
        maskLocation <<= 1;                     // shift maskLocation
    }
    x = (x | maskLocation);                     // changes 0 to 1 at only masklocation
